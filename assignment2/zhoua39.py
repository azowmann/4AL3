...
def plot_all_confusion_matrices(all_confusion_matrices, feature_combinations):
    ...
    fig, axes = plt.subplots(total_combos, 10, figsize=(20, 3 * total_combos))

    axes = axes.flatten() # flatten to account for numpy 
    ...
    for combo_index, (combo_name, confusion_matrices) in enumerate(all_confusion_matrices.items()):
        for fold_index, cm in enumerate(confusion_matrices):
            ...
            disp.plot(ax=axes[combo_index * 10 + fold_index], colorbar=False)  
            axes[combo_index * 10 + fold_index].set_title(f"Fold: {fold_index+1}", fontsize=6)
        axes[combo_index * 10].set_ylabel(f"{combo_name}", fontsize=8, rotation=0, labelpad=60, va='center')
